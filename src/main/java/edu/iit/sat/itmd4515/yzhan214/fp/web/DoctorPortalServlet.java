 ...
 * @author gxh19_000
 */
@WebServlet(name = "DoctorPortalServlet", urlPatterns = {"/d/DoctorPortalServlet"})
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if(request.isUserInRole("doctor")) {
              Doctor doc = doctorService.findByUsername(request.getRemoteUser());
              out.println("<h2>" + doc.getFirstName()+" "+doc.getLastName() + "</h2>");
              request.setAttribute("doctor", doc);
              List<Doctor> docList = doctorService.findAll();
              request.setAttribute("doctors", docList);
            }
            
            request.getRequestDispatcher("/WEB-INF/pages/doctor.jsp").forward(request, response);
        }
    }
